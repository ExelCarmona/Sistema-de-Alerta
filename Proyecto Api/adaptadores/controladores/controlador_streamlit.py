...
# ─────────────────────────────────────────
#  DICCIONARIO DE CÓDIGOS CLIMÁTICOS
# ─────────────────────────────────────────
CODIGOS_CLIMA = {
...
def formatear_etiqueta_localizacion(localizacion: Localizacion) -> str:
    zona = localizacion.zona_horaria or "?"
    return f"📍 Lat {localizacion.latitud:.4f} / Lon {localizacion.longitud:.4f} — {zona} (ID {localizacion.id})"

# ─────────────────────────────────────────
#  SECCIONES DE LA INTERFAZ
# ─────────────────────────────────────────

def mostrar_dashboard(caso_consultar: CasoUsoConsultarClima):
    ...
    localizaciones = caso_consultar.obtener_localizaciones()
    ...
    if not localizaciones:
        ...
        registro = caso_consultar.obtener_registro_completo(id_localizacion, limite_horas=168, limite_dias=14)
        ...
        if registro:
            ...
            with col_graf1:
                ...
                if registro.clima_horario:
                    ...
                    # Filtramos nulos
                    datos_grafico = [(t, v) for t, v in zip(tiempos, temps) if v is not None]
                    if datos_grafico:
...
def mostrar_sincronizacion(caso_sincronizar: CasoUsoSincronizarClima):
    ...
    if submitted:
        with st.spinner("⏳ Conectando con la API de Open-Meteo y guardando datos..."):
            try:
                registro = caso_sincronizar.ejecutar(latitud=latitud, longitud=longitud)
...
def mostrar_registro_manual(caso_registrar: CasoUsoRegistrarClimaManual, caso_consultar: CasoUsoConsultarClima):
    ...
    with tipo_registro[0]:
        ...
        if sub:
            try:
                ...
                new_id = caso_registrar.registrar_localizacion(loc)
            ...
            if sub_a:
                try:
                    ca = ClimaActual(
                        ...
                        id_localizacion=opciones[sel_loc]
                    )
                    caso_registrar.registrar_clima_actual(ca)
            ...
            if sub_h:
                try:
                    ch = ClimaHorario(
                        ...
                        precipitacion=precip_h, codigo_clima=cod_h, id_localizacion=opciones[sel_loc_h]
                    )
                    caso_registrar.registrar_clima_horario(ch)
            ...
            if sub_d:
                try:
                    cd = ClimaDiario(
                        ...
                        suma_precipitacion=precip_d, id_localizacion=opciones[sel_loc_d]
                    )
                    caso_registrar.registrar_clima_diario(cd)
                    st.success(f"✅ Registro diario para `{fecha_d}` guardado correctamente.")
                except Exception as e:
                    st.error(f"❌ Error: {e}")


def mostrar_recomendaciones(caso_recomendaciones: CasoUsoObtenerRecomendaciones, caso_consultar: CasoUsoConsultarClima):
...
# ─────────────────────────────────────────
#  FUNCIÓN PRINCIPAL (ARRANQUE DE INTERFAZ)
# ─────────────────────────────────────────
def arrancar_interfaz(casos_uso: dict):
    # Inyección de estilos CSS globales y premium
    st.markdown("""
    <style>
    ...
    if seccion == "📊 Dashboard de Clima":
        mostrar_dashboard(casos_uso["consultar"])
    elif seccion == "🌐 Sincronizar desde API":
        mostrar_sincronizacion(casos_uso["sincronizar"])
    elif seccion == "✍️ Registro Manual":
        mostrar_registro_manual(casos_uso["registrar_manual"], casos_uso["consultar"])
    elif seccion == "🌱 Recomendaciones de Cultivo":
        mostrar_recomendaciones(casos_uso["recomendaciones"], casos_uso["consultar"])
